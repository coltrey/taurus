...
RUN curl https://jmeter-plugins.org/repo/ | python3 -c 'import json; import sys; sys.stdout.write(",".join([x["id"] for x in json.loads(sys.stdin.read()) if "id" in x.keys() and not x["id"].startswith("ulp")]))' > plugins.lst \
    cat plugins.lst
